...
nr_channels = 4
lambda_phi = 1.0  # = capac ?
lambda_chi = 0.0  # = capac ?
ksi_0 = 10.0
...
def A_plus(t_array, beta, bias, lambda_phi, lambda_chi, ksi_0):
    ...
    if bias != 0.0:
        h = integrate.cumulative_trapezoid(
            x=t_array, y=h_integrand(t_array), initial=0.0
        )
    else:
        h = 0.0
    ...
    left_chunk = np.divide(
        1j * np.pi * np.ones_like(t_array),
        (beta * ksi_0 * np.sinh(np.pi * t_array / beta)),
        where=t_array != 0.0,
    )
    left_chunk[t_array == 0.0] = np.nan
